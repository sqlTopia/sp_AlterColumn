...
GO
IF OBJECT_ID(N'tools.atac_configurations', N'U') IS NOT NULL
        DROP TABLE tools.atac_configurations;
GO
CREATE TABLE    tools.atac_configurations
                (
                        tag VARCHAR(36) NOT NULL CONSTRAINT df_tools_atac_configurations_tag DEFAULT (''),
                        table_name VARCHAR(257) NOT NULL CONSTRAINT ck_tools_atac_configurations_table_name CHECK (table_name > ''),
                        column_name VARCHAR(128) NOT NULL CONSTRAINT ck_tools_atac_configurations_column_name CHECK (column_name > ''),
                        new_column_name VARCHAR(128) NULL CONSTRAINT ck_tools_atac_configurations_new_column_name CHECK       (
                                                                                                                                   new_column_name IS NULL      -- Inherit current column setting
                                                                                                                                OR new_column_name > ''         -- Change column name 
                                                                                                                        ),
                        is_nullable VARCHAR(5) NULL CONSTRAINT ck_tools_atac_configurations_is_nullable CHECK (
                        ...
                        datatype_name VARCHAR(128) NULL CONSTRAINT ck_tools_atac_configurations_datatype_name CHECK   (
                                                                                                                           datatype_name IS NULL        -- Inherit current column setting
                                                                                                                        OR datatype_name > ''           -- Set new datatype name
                                                                                                                ),
                        max_length VARCHAR(4) NULL CONSTRAINT ck_tools_atac_configurations_max_length CHECK   (
                        ...
                        precision TINYINT NULL CONSTRAINT ck_tools_atac_configurations_precision CHECK        (
                                                                                                                   precision IS NULL                    -- Inherit current column setting
                                                                                                                OR precision >= 1 AND precision <= 38   -- Set new precision
                                                                                                        ),
                        scale TINYINT NULL CONSTRAINT ck_tools_atac_configurations_scale CHECK        (
                                                                                                           scale IS NULL        -- Inherit current column setting
                                                                                                        OR scale <= 38          -- Set new scale
                                                                                                ),
                        collation_name VARCHAR(128) NULL CONSTRAINT ck_tools_atac_configurations_collation_name CHECK (
                                                                                                                           collation_name IS NULL       -- Inherit current setting
                                                                                                                        OR collation_name > ''          -- Set new collation name
                                                                                                                ),
                        xml_collection_name VARCHAR(257) NULL CONSTRAINT ck_tools_atac_configurations_xml_collection_name CHECK       (
                        ...
                        datatype_default_name VARCHAR(128) NULL CONSTRAINT ck_tools_atac_configurations_default_name CHECK    (
                        ...
                        datatype_rule_name VARCHAR(128) NULL CONSTRAINT ck_tools_atac_configurations_rule_name CHECK  (
                        ...
                        log_text VARCHAR(MAX) NULL CONSTRAINT ck_tools_atac_configurations_log_text CHECK (log_text > ''),
                        CONSTRAINT ck_tools_atac_configurations_table_name_column_name CHECK (COLUMNPROPERTY(OBJECT_ID(table_name, 'U'), column_name, 'ColumnId') IS NOT NULL),
                        CONSTRAINT ck_tools_atac_configurations_datatype_name_max_length_precision_scale CHECK        (
                        ...
                        CONSTRAINT ck_tools_atac_configurations_datatype_name_collation_name CHECK    (
                        ...
                        CONSTRAINT ck_tools_atac_configurations_datatype_name_xml_collection_name CHECK       (
                        ...
                        CONSTRAINT pk_tools_atac_configurations PRIMARY KEY CLUSTERED (table_name, column_name, tag)
                );
GO
