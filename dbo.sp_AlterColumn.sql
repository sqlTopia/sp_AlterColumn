...
BEGIN TRY
        ...
        */
        ...
        -- Clean up
        SELECT  @sql = STRING_AGG(CAST(CONCAT('EXEC msdb.dbp.sp_delete_job @job_name = ', QUOTENAME(jbs.name COLLATE DATABASE_DEFAULT, ''''), ', @delete_unused_schedule = 1;') AS VARCHAR(MAX)), '; ') WITHIN GROUP (ORDER BY jbs.job_id)
